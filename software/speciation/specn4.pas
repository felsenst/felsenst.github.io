   ...
   procedure getnextgen;
      ...
      procedure recombine;
         var i, j, k, l, n : integer;
             x, x1, p, delt : real;
      begin {maybe this doesn't work if locus a in diseq. with others}
         for l := 1 to 2 do begin
            ...
            for i := 1 to gamnumber do
               for j := 1 to gamnumber do begin 
                  delt := delta + (contents[i,1] + contents[j,1] - 2) * ddelt;
                  if contents[i,2] = contents[j,2]
                  then begin
                     if contents[i,2] = 1
                     then x := 1 - delt + delt/p
                     else x := 1 - delt + delt/(1.0-p);
                     end
                  else x := 1 - delt;
...
begin
   ...
   while continue = TRUE do begin
      ...
      if s>0 then begin
         ...
         write('delta? ');
         readln(delta);
         write('effect of modifier of delta?');
         readln(ddelt);
         writeln('recombination fractions: ');
         for i := 1 to lociminusone do begin
            write('  ',i-1,' -',i:2,' ?');
            readln(r[i]);
            end;
