...
program specn;
   ...
   var i, j, k, l, m, n : integer;
      ...
      contents : array[1..gamnumber, 1..loci] of integer;
      contrib : array[1..gamnumber, 1..gamnumber] of integer;
   ...
   procedure setuppop;
      var i, j : integer;
         prod, p2, p3 : real;
   begin
      write('prob B given A ? ');
      readln(p2);
      write('prob C given A ? ');
      readln(p3);
      for i := 1 to gamnumber do begin
         prod := 0.5;
         if contents[i,2] = contents[i,1]
         then prod := prod * p2
         else prod := prod * (1.0-p2);
         if contents[i,3] = contents[i,1]
         then prod := prod * p3
         else prod := prod * (1.0 - p3);
         gambefore[1,i] := prod;
         gambefore[2,i] := prod;
         end;
   end;
         ...
         for l := 1 to 2 do begin
            ...
            for i := 1 to gamnumber do
               for j := 1 to gamnumber do begin 
                  ...
                  x := x * gamafter[l,i] * gamafter[l,j];
                  for k := 1 to gamnumber do begin
                     x1 := x * recfrac[k];
                     n := contrib[i,k] +
                      contrib[j,gamnumber-k+1] + 1;
                     gambefore[l,n] := gambefore[l,n] + x1;
                     end;
                  end;
               end;
      end;
