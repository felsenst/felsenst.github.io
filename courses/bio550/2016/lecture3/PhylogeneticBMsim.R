...
##Three different ways of simulating a tree
#tree=rtree(ntaxa)
#tree=rcoal(ntaxa)
tree=birthdeath.tree(b=1,d=0,taxa.stop=ntaxa)

##Plot the tree
plot(tree)
trait.values=fastBM(tree)
...
plot(tree)
axisPhylo()
tiplabels(pch=19, col=mycol(trait.values), cex=(scale(trait.values)+3)/2)

##Write the tree to newick format
write.tree(tree,file="treefile.newick")

##Write the trait values to a data file to be read by phylip
names=paste(tree$tip,"          ",sep="")
trait.values=as.matrix(trait.values)
concatenate=function(d){
  x=NULL
  if(length(d)>1){
  for (i in 1:length(d)){
    x=paste(x,round(d[i],5)," ")
    
  }
print(x)
} else {round(d,5)}
}
concatenate(trait.values[1,])

write(paste(ntaxa,ntraits," "),file="BMsims.out")
for (i in 1:ntaxa){
  write(paste(names[i], concatenate(trait.values[i,])),append=TRUE,file="BMsims.out")
}


##Simulate Multiple Characters from a G-matrix
G=matrix(c(1,-0.9,-0.9,1),nrow=2,ncol=2)
G
traits=sim.char(tree,G)[,,1]
##Plot the tree and two trait values
plot(tree)
axisPhylo()
tiplabels(pch=19, col=mycol(traits[,1]), cex=(scale(traits[,1])+3)/2,adj=0.6)
tiplabels(pch=19, col=mycol(traits[,2]), cex=(scale(traits[,2])+3)/2, adj=0.7)
