...
{ Shimodaira AU calculation like complete-and-partial calculation }
   const Pvalue = 0.95;
...
begin
   ...
   repeat
     ...
     readln(Pc);
     writeln('partial bootstrap fraction?');
     readln(Pp);
     P := phi(-3.0*invnormal(1.0-Pc)+2.0*sqrt(2.0)*invnormal(1.0-Pp));
     writeln('   original   corrected');
     writeln(Pc:10:2, P:10:4);
   until (Pc < 0.0);
end.
